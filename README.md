...
**Production** (`NODE_ENV=production` or `APP_ENV=production`) does not load a dotenv file. Set `JWT_SECRET`, `EMAIL_PEPPER`, `PORT`, `SQLITE_PATH`, and optionally `DEFAULT_USER_EMAIL` / `DEFAULT_USER_PASSWORD` on the host. See `server/.env.production.example`. The API serves `client/dist` from the same origin after `npm run build && npm start`. The cloud provider is still TBD.
